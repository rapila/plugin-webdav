...
class HTTP_WebDAV_Server
{
    ...
    function ServeRequest()
    {
        ...
        if(!$this->uri) {
          $this->uri = $this->base_uri.$path_info;
        }
    ...
    */
    ...
    function http_PUT()
    {
        if ($this->_check_lock_status($this->path)) {
            $options                   = Array();
            $options["path"]           = $this->path;
            $options["content_length"] = isset($this->_SERVER["CONTENT_LENGTH"]) ? $this->_SERVER["CONTENT_LENGTH"] : 0;
            ...
            if($stat === false) {
                ...
                if (!empty($options["ranges"])) {
                    ...
                    while(!feof($options["stream"])) {
                      stream_copy_to_stream($options["stream"], $stream);
                    }
                }
    ...
    /**
     * Unslashify - make sure path doesn't end in a slash
     *
     * @param   string directory path
